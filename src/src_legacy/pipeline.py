...
def main(pathogen_code, data_dir):
    """
    Pipeline to get datasets for a given pathogen for which data has already been fetched.
    """
    pathogen = pathogen_code
    
    print("Processing data for {}".format(pathogen))
    df = pd.read_csv(os.path.join(data_dir, pathogen, "{}_original.csv".format(pathogen)), low_memory=False)
    initial_len = len(df)
    rc = RawCleaner()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Generate datasets for a given pathogen')
    parser.add_argument('--pathogen_code', '-p', type=str, help='Pathogen code (e.g. abaumannii)', required=True)
    parser.add_argument('--dir', '-d', type=str, help='Directory where the data is stored', required=False)
    args = parser.parse_args()

    main(args.pathogen_code, args.dir)